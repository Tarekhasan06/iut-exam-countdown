...
export async function insertStudyMaterial(material: InsertStudyMaterial) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
  ...
  const created = await db
    .select()
    .from(studyMaterials)
    .where(and(eq(studyMaterials.userId, material.userId), eq(studyMaterials.fileKey, material.fileKey)))
    .limit(1);

  return created[0];
}

export async function deleteStudyMaterial(userId: number, id: number) {
  const db = await getDb();
  if (!db) throw new Error("Database is not available");
